...
multilib_get_enabled_abi_pairs() {
	...
	if [[ ! ${found} ]]; then
		...
		debug-print "${FUNCNAME}: no ABIs enabled, fallback to ${abi} or ${ARC_MULTILIBS}"
		debug-print "${FUNCNAME}: ABI=${ABI}, DEFAULT_ABI=${DEFAULT_ABI}"
		if [[ -n "${ARC_MULTILIBS}" ]]; then
			for abi in ${ARC_MULTILIBS}; do
			  echo ".${abi}"
			done
		else
		  einfo "No ARC_MULTILIBS fallback to default"
		  echo ".${abi}"
		fi
	fi
}
